...
import { useReportPost } from '@/api/reports';
...
export const ReportPostBottomSheet = React.forwardRef<BottomSheetModal, Props>(
  ({ postId, onClose }, optionsRef) => {
    const { mutate } = useReportPost({
      onMutate: () => {
        onClose();
      },
    });
      ...
      (option: Option) => {
        mutate(
          ...
          },
          {
            onSuccess: () => {
              showSuccessMessage(
                "Thank you for your report! We'll review the feed to ensure it meets our community guidelines."
              );
            },
          }
        );
