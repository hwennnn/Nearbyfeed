...
const getPostAgeHours = (post: Post, now = Date.now()): number => {
...
export const getPostPulseScore = (post: Post, now = Date.now()): number => {
  ...
  const locationBonus = post.location !== null && post.location !== undefined ? 4 : 0;
...
export const getPostSignal = (post: Post, now = Date.now()): FeedSignal => {
...
export const getFeaturedFeedPost = (
  posts: Post[],
  now = Date.now(),
): Post | undefined =>
  [...posts].sort((a, b) => getPostPulseScore(b, now) - getPostPulseScore(a, now))[0];

const getLiveLensMeta = (post: Post): string => {
  ...
  if (post.location?.name !== undefined) return `near ${post.location.name}`;
...
const getLiveUpdateTime = (update: LiveUpdate): number => {
...
export const getFeedSceneBoard = (
  ...
  const postMoments: FeedSceneMoment[] = getFeedLiveLensItems(posts, now, 3).map(
    (item) => ({
      id: `post-${item.id}`,
      kind: 'post',
      meta: item.meta,
