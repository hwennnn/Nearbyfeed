...
export type PollPreviewInput = {
  createdAt?: PollLifecycleTimestamp;
  options: PollOptionResultInput[];
...
export type PollPreviewStatusTone = 'live' | 'closing' | 'closed' | 'voted';
...
export type PollPreviewModel = {
  ...
  statusLabel: string;
  statusTone: PollPreviewStatusTone;
  totalOptionLabel: string;
  voteLabel: string;
};

const POLL_CLOSING_SOON_MS = 6 * 60 * 60 * 1000;

export const getPollPreviewStatus = (
  poll: PollPreviewInput,
  ...
  return {
    ...
    statusTone: remainingMs <= POLL_CLOSING_SOON_MS ? 'closing' : 'live',
  };
};
...
export const getPollPreviewModel = (
  ...
  const status = getPollPreviewStatus(poll, now);
